...
// --- Phone-number login OTP — the most brute-forceable surface on the
// public site (6-digit code = 1,000,000 combinations, but still finite).
// Limited per-phone (stops guessing one shopper's code) AND per-IP (stops
// spamming OTP sends to arbitrary numbers, which costs real SMS credits). --
const phoneLoginPhoneLimiter = makeUpstashLimiter(15, "15 m", "phone-login-phone");
const phoneLoginIpLimiter = makeUpstashLimiter(60, "60 m", "phone-login-ip");

/**
 * @param {{ phone: string, ip: string }} params
 * @returns {Promise<{ success: boolean }>}
 */
export async function checkPhoneLoginRateLimit({ phone, ip }) {
  try {
    const phoneOk = phoneLoginPhoneLimiter
      ? (await phoneLoginPhoneLimiter.limit(phone)).success
      : memoryLimit(`phone-login-phone:${phone}`, 15, 15 * 60 * 1000);
    const ipOk = phoneLoginIpLimiter
      ? (await phoneLoginIpLimiter.limit(ip)).success
      : memoryLimit(`phone-login-ip:${ip}`, 60, 60 * 60 * 1000);
    return { success: phoneOk && ipOk };
  } catch (err) {
    console.error("[rate-limit] phone-login check failed, allowing request:", err);
    return { success: true };
  }
}

const verifyPhoneLimiter = makeUpstashLimiter(30, "15 m", "verify-phone");

/**
 * @param {{ phone: string, ip: string }} params
 * @returns {Promise<{ success: boolean }>}
 */
export async function checkPhoneOtpVerifyRateLimit({ phone, ip }) {
  try {
    const phoneOk = verifyPhoneLimiter
      ? (await verifyPhoneLimiter.limit(phone)).success
      : memoryLimit(`verify-phone:${phone}`, 30, 15 * 60 * 1000);
    return { success: phoneOk };
  } catch (err) {
    console.error("[rate-limit] verify check failed, allowing request:", err);
    return { success: true };
  }
}
