...
const phoneLoginPhoneLimiter = makeUpstashLimiter(5, "15 m", "phone-login-phone");
const phoneLoginIpLimiter = makeUpstashLimiter(20, "60 m", "phone-login-ip");
...
export async function checkPhoneLoginRateLimit({ phone, ip }) {
  try {
    const phoneOk = phoneLoginPhoneLimiter
      ...
      : memoryLimit(`phone-login-phone:${phone}`, 5, 15 * 60 * 1000);
    const ipOk = phoneLoginIpLimiter
      ? (await phoneLoginIpLimiter.limit(ip)).success
      : memoryLimit(`phone-login-ip:${ip}`, 20, 60 * 60 * 1000);
    return { success: phoneOk && ipOk };
  } catch (err) {
