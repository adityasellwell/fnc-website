...
export async function sendSms(templateKey, mobile, vars = {}) {
  ...
  try {
    ...
    // PowersText's SSL certificate chain is missing intermediate CA certs,
    // causing Node.js native fetch() to reject with UNABLE_TO_VERIFY_LEAF_SIGNATURE.
    // Using node https module with rejectUnauthorized: false bypasses this chain error.
    const result = await new Promise((resolve, reject) => {
      const req = https.get(fullUrl, { rejectUnauthorized: false, timeout: 10000 }, (res) => {
        ...
        res.on("end", () => {
          ...
          resolve({ ok: res.statusCode >= 200 && res.statusCode < 300, data, raw: body });
        });
      });
      req.on("error", reject);
      req.on("timeout", () => {
    ...
    const failed = !result.ok || data?.status?.toLowerCase() !== "success";
    if (failed) {
      console.error(`[SMS] ${templateKey} to ${mobileNumber} failed:`, data || result.raw);
      return { success: false, error: data || result.raw };
    }
    console.log(`[SMS SUCCESS] ${templateKey} sent to ${mobileNumber}:`, data?.["message-id"] || data?.message);
