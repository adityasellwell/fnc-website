...
/**
 * Customer sign-in/sign-up "Phone & OTP" tab. OTPs are sent through our
 * own SMS gateway (lib/sms.js, app/api/auth/phone-otp/*) — not Firebase's
 * native phone auth, which required their paid Blaze plan and showed a
 * reCAPTCHA badge. Firebase is only used behind the scenes (a custom
 * token) to mint the actual login session once our own OTP is verified.
 *
 * Not related to the delivery-rider handoff OTP (services/orders.js) —
 * that one is shown in-app on the customer's tracking page.
 */
...
export const FOOTER_LINKS = {
  shop: [
    ...
    { label: "Dine-in / Takeaway Menu", href: "/menu" },
...
export const CATEGORY_META = {
  ...
  marinated: { icon: "Utensils", tone: "red", image: "/images/categories/marinated.jpg" },
  "protein-salads": { icon: "Leaf", tone: "green", image: "/images/categories/protein-salads.jpg" },
  "healthy-snacks": { icon: "Apple", tone: "neutral", image: "/images/categories/healthy-snacks.jpg" },
  "healthy-protein-drinks": { icon: "GlassWater", tone: "blue", image: "/images/categories/healthy-protein-drinks.jpg" },
  "fresh-protiens-more": { icon: "Sparkles", tone: "red", image: "/images/categories/fresh-proteins-more.jpg" },
};

/**
 * Looks up CATEGORY_META for a product's immediate category, falling back
 * to its PARENT category's entry when the immediate one is a subcategory
 * (e.g. "Raw"/"Snacks" under "Chicken") with no icon/image of its own.
 * Without this, every subcategory added anywhere on the site would need
 * its own duplicate CATEGORY_META entry or silently render no icon/image —
 * this makes new subcategories (any name, under any category) work for
 * free by inheriting their parent's look.
 */
export function resolveCategoryMeta(slug, parentSlug) {
  return CATEGORY_META[slug] ?? CATEGORY_META[parentSlug] ?? { icon: "Fish", tone: "red" };
}
