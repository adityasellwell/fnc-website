...
/** Sent once payment is confirmed via webhook — the real "order confirmed" moment. */
export async function sendOrderConfirmedEmail(customer, order) {
  return sendEmail({
    to: customer.email,
    subject: `Order Confirmed — #${order.id.slice(-8).toUpperCase()}`,
    html: baseLayout(`
      <h2 style="font-size: 20px; color: #059669; margin: 0 0 8px;">Your order is confirmed ✓</h2>
      <p style="color: ${brand.muted}; margin: 0 0 24px;">We've received your payment and your order is now with our team.</p>
      <div style="background: #f9f9f9; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
        <p style="margin: 0; font-size: 14px;"><strong>Order:</strong> #${order.id.slice(-8).toUpperCase()}</p>
        <p style="margin: 8px 0 0; font-size: 14px;"><strong>Total:</strong> ₹${Number(order.total).toFixed(2)}</p>
      </div>
      <p style="color: ${brand.muted}; font-size: 13px;">Track your order anytime from your <a href="${process.env.NEXT_PUBLIC_APP_URL || "#"}/account/orders/${order.id}" style="color: ${brand.primary};">order page</a>.</p>
    `),
  }).catch(() => {});
}

const STATUS_EMAIL_COPY = {
  PREPARING: { headline: "Our team is fresh-packing your order", desc: "We've started preparing your items." },
  OUT_FOR_DELIVERY: { headline: "Your order is on its way!", desc: "It's out for delivery — keep an eye out." },
  READY_FOR_PICKUP: { headline: "Your order is ready for pickup", desc: "Head to the store whenever it's convenient." },
};

/** Generic status-update nudge for mid-flow statuses that aren't already covered by a dedicated template. */
export async function sendOrderStatusUpdateEmail(customer, order, status) {
  const copy = STATUS_EMAIL_COPY[status];
  if (!copy) return;
  return sendEmail({
    to: customer.email,
    subject: `${copy.headline} — Order #${order.id.slice(-8).toUpperCase()}`,
    html: baseLayout(`
      <h2 style="font-size: 20px; color: ${brand.text}; margin: 0 0 8px;">${copy.headline}</h2>
      <p style="color: ${brand.muted}; margin: 0 0 24px;">${copy.desc}</p>
      <p style="color: ${brand.muted}; font-size: 13px;">Track live status on your <a href="${process.env.NEXT_PUBLIC_APP_URL || "#"}/account/orders/${order.id}" style="color: ${brand.primary};">order page</a>.</p>
    `),
  }).catch(() => {});
}
...
// ─────────────────────────────────────────────────────────────
// Admin/owner notifications — franchise applications and contact
// messages both saved to the database with no way for the owner to
// find out a new one arrived (no admin list page, no email). This is
// the fix for that: a fire-and-forget email to the business address
// the moment either form is submitted, so a real inquiry is never
// silently sitting unread in the database.
// ─────────────────────────────────────────────────────────────

async function resolveNotifyEmail() {
  // INITIAL_ADMIN_EMAIL first, not businessInfo.email: Settings currently
  // still holds its seeded placeholder ("hello@fncfresh.in", not a real
  // inbox) since nobody has edited it in Admin -> Settings yet. Silently
  // sending notifications into a placeholder address would look like it
  // worked while actually going nowhere — safer to trust the env var,
  // which was set to a real address during setup.
  if (process.env.INITIAL_ADMIN_EMAIL) return process.env.INITIAL_ADMIN_EMAIL;
  try {
    const { getSettings } = await import("@/services/settings");
    const settings = await getSettings();
    return settings?.businessInfo?.email || null;
  } catch {
    return null;
  }
}

/** Sent to the business owner the moment someone submits /franchise/apply */
export async function sendFranchiseLeadNotification(lead) {
  const to = await resolveNotifyEmail();
  if (!to) return;
  return sendEmail({
    to,
    subject: `New franchise inquiry — ${lead.name} (${lead.city})`,
    html: baseLayout(`
      <h2 style="font-size: 20px; color: ${brand.text}; margin: 0 0 16px;">New Franchise Application</h2>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px; color: ${brand.text};">
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Name</td><td style="padding: 6px 0; font-weight: 600;">${lead.name}</td></tr>
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Email</td><td style="padding: 6px 0;"><a href="mailto:${lead.email}" style="color: ${brand.primary};">${lead.email}</a></td></tr>
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Phone</td><td style="padding: 6px 0;"><a href="tel:${lead.phone}" style="color: ${brand.primary};">${lead.phone}</a></td></tr>
        <tr><td style="padding: 6px 0; color: ${brand.muted};">City</td><td style="padding: 6px 0;">${lead.city}</td></tr>
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Investment Budget</td><td style="padding: 6px 0;">${lead.investmentBudget}</td></tr>
      </table>
      ${lead.message ? `<p style="margin: 16px 0 0; padding-top: 16px; border-top: 1px solid #f0f0f0; color: ${brand.text};">${lead.message}</p>` : ""}
    `),
  }).catch(() => {});
}

/** Sent to the business owner the moment someone submits the Contact form */
export async function sendContactMessageNotification(message) {
  ...
  return sendEmail({
    ...
    html: baseLayout(`
      <h2 style="font-size: 20px; color: ${brand.text}; margin: 0 0 16px;">New Contact Message</h2>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px; color: ${brand.text};">
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Name</td><td style="padding: 6px 0; font-weight: 600;">${message.name}</td></tr>
        <tr><td style="padding: 6px 0; color: ${brand.muted};">Email</td><td style="padding: 6px 0;"><a href="mailto:${message.email}" style="color: ${brand.primary};">${message.email}</a></td></tr>
        ${message.phone ? `<tr><td style="padding: 6px 0; color: ${brand.muted};">Phone</td><td style="padding: 6px 0;"><a href="tel:${message.phone}" style="color: ${brand.primary};">${message.phone}</a></td></tr>` : ""}
      </table>
      <p style="margin: 16px 0 0; padding-top: 16px; border-top: 1px solid #f0f0f0; color: ${brand.text};">${message.message}</p>
    `),
  }).catch(() => {});
}
