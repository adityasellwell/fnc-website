...
function mapProduct(product) {
  ...
  return {
    ...
    // The immediate category may be a subcategory (e.g. "Raw" under
    // "Chicken") that has no icon/fallback-image entry of its own in
    // CATEGORY_META — this lets every consumer fall back to the parent's
    // entry instead of silently rendering no icon/image at all. Null for
    // top-level categories (no parent to fall back to).
    parentCategorySlug: product.category?.parentCategory?.slug ?? null,
    ...
    variants: (product.variants ?? []).map((v) => ({
      ...
      label: v.variantOption?.label ?? "",
      type: v.variantOption?.type ?? "Weight",
    })),
...
const PRODUCT_INCLUDE = {
  category: { select: { slug: true, parentCategory: { select: { slug: true } } } },
...
export async function getProducts() {
  try {
    const products = await db.product.findMany({
      // Admin-toggled visibility — an Inactive product is fully editable in
      // the admin panel (services/products.js queries separately, unfiltered)
      // but hidden from every public listing/detail page/search here.
      where: { isActive: true },
...
export async function getProductBySlug(slug) {
  try {
    ...
    // isActive checked after the fetch (not in the where clause) so a
    // direct link to a deactivated product 404s the same way a wrong slug
    // does, rather than leaking a Prisma-vs-not-found distinction.
    return product && product.isActive ? mapProduct(product) : null;
  } catch {
    return MOCK_PRODUCTS.find((p) => p.slug === slug) ?? null;
  }
}

// extraSlugs rolls a parent category's product listing up to include its
// subcategories too (e.g. viewing "Fish" shows products from "Raw" and
// "Snacks" as well) — pass the child slugs already resolved by the caller
// (lib/data/categories.js's getCategoryChildren), this function doesn't
// look them up itself to avoid a second DB round-trip per call.
export async function getProductsByCategory(categorySlugOrId, extraSlugs = []) {
  const slug = categorySlugOrId.replace(/^cat-/, "");
  const slugs = [slug, ...extraSlugs];
  try {
    const products = await db.product.findMany({
      where: { isActive: true, category: { slug: { in: slugs } } },
      include: PRODUCT_INCLUDE,
      orderBy: { createdAt: "asc" },
    });
    return products.map(mapProduct);
  } catch {
    return MOCK_PRODUCTS.filter((p) => slugs.includes(p.categoryId.replace(/^cat-/, "")));
  }
}
